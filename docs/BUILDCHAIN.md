...
> **Publish path updated (2026-07).** apk packages are now published to
> **Cloudsmith** (`asterisk/alpine`), not GitHub Pages. The publish / Pages /
> `APKINDEX`-deploy details below describe the **retired** Pages setup - Cloudsmith
> now owns indexing, signing, and hosting. Current source of truth: the
> [main README](../README.md) and `.github/workflows/_publish.yml`. The local build
> + test internals (including local `make repo-index-*` for `make repo-server`) are
> unchanged.
...
- **Automated build + test** via Makefile, and a GitHub Actions pipeline that publishes to Cloudsmith (was GitHub Pages, retired 2026-07)
...
├── .github/workflows/ci.yml    # Build + test + publish to Cloudsmith
├── docker-compose.yml
├── Makefile
...
`.github/workflows/ci.yml` builds, smoke-tests, and publishes the repo:
...
- **publish** pushes the built APKs to Cloudsmith, which signs + indexes them (on push to `main`/tags only; was: sign the merged index + deploy to GitHub Pages, retired 2026-07)
